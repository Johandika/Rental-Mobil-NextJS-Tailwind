...
export async function fetchCars(filters: FilterProps) {
  ...
  const headers = {
    "X-RapidAPI-Key": "3945351cedmshd65ff396a34543cp114bebjsnc575c0cdc39a",
...
export const calculateCarRent = (city_mpg: number, year: number) => {
  ...
  const mileageFactor = 0.1; // Tarif tambahan per mil yang ditempuh
  const ageFactor = 0.05; // Tarif tambahan per tahun umur kendaraan
